...
/**
 * GET /api/tastytrade/balances
 *
 * Returns the account balances for the authenticated user.
...
async function fetchBalances(accessToken: string, accountNumber: string) {
    return fetch(`${TASTY_BASE_URL}/accounts/${accountNumber}/balances`, {
        headers: {
            Authorization: `Bearer ${accessToken}`,
            Accept: "application/json",
            "User-Agent": TASTY_HEADERS["User-Agent"],
...
export async function GET(req: NextRequest) {
    ...
    let upstream = await fetchBalances(accessToken, accountNumber);
    ...
    if (upstream.status === 401) {
        ...
        upstream = await fetchBalances(newToken, accountNumber);
    }

    if (!upstream.ok) {
        const detail = await upstream.text();
        return NextResponse.json(
            { ok: false, error: "Failed to fetch balances", status: upstream.status, detail },
    ...
    return NextResponse.json(
        { ok: true, accountNumber, balances: data?.data ?? data },
