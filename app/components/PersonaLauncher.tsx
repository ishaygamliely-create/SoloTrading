...
export function PersonaLauncher({ onClick, isOpen, hasActivePersona }: Props) {
    return (
        <button
            onClick={onClick}
            className={`w-full group relative overflow-hidden rounded-xl border transition-all duration-300 p-4 flex items-center justify-between mt-2
                ${isOpen
        ...
        >
            ...
            <div className="flex items-center gap-3 relative z-10">
                <div className={`p-2 rounded-lg border transition-colors ${isOpen ? 'bg-white/20 border-white/30' : 'bg-blue-500/10 border-blue-500/20 group-hover:bg-blue-500/20'}`}>
                    <Brain size={16} className={isOpen ? 'text-white' : 'text-blue-400'} />
                </div>
                <div className="flex flex-col text-left">
                    ...
                    <span className={`text-[9px] font-bold uppercase mt-1 tracking-wider ${isOpen ? 'text-blue-100' : 'text-zinc-500'}`}>
                        {hasActivePersona ? 'Active Engine' : 'Adaptive Style'}
                    </span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 relative z-10">
                <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[9px] font-black uppercase transition-all
                    ...
                    {isOpen ? 'ACTIVE' : hasActivePersona ? 'CONFIGURED' : 'OPEN'}
